...
class CA_World(OnOffWorld):
    ...
    def build_initial_line(self):
        ...
        if self.init == 'Random':
            # Set the initial row to random on/off
    ...
    def handle_event_and_values(self):
        ...
        elif event in {OnOffWorld.SELECT_ON_TEXT, OnOffWorld.SELECT_OFF_TEXT}:
    ...
    def make_switches_and_rule_nbr_consistent(self):
        ...
        if self.rule_nbr != new_switches_nbr:
            ...
            self.rule_nbr = SimEngine.get_gui_value('Rule_nbr')
            self.set_switches_from_rule_nbr()
        self.set_binary_nbr_from_rule_nbr()
    ...
    def set_display_from_lines(self):
        ...
        case in which there are more or fewer lines than Patch row.
        """
        nbr_ca_lines = len(self.ca_lines)
        first_ca_line_to_display = max(0, nbr_ca_lines - CA_World.ca_display_size)
        self.init = SimEngine.get_gui_value('init')
        starting_pos_in_line = 0 if self.init == 'Left' else \
                               (len(self.ca_lines[0])-CA_World.ca_display_size)//2  if self.init == 'Center' else \
                               len(self.ca_lines[0])-CA_World.ca_display_size    #  'Right'
        patches_row_nbr = max(0, CA_World.ca_display_size - nbr_ca_lines)
        for line in self.ca_lines[first_ca_line_to_display:]:
            for col_nbr in range(gui.PATCH_COLS):
                patch = CA_World.patches_array[patches_row_nbr, col_nbr]
                patch.set_on_off(line[starting_pos_in_line + col_nbr])
            patches_row_nbr += 1
        gui.WINDOW['rows'].update(value=len(self.ca_lines))
